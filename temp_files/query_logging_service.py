...
class QueryLoggingService:
    ...
    def __init__(self):
        self._pending_logs: List[GatewayQueryLog] = []
        self._batch_size = 10
    ...
    async def log_query(
        ...
        try:
            async for db in get_platform_db():
    ...
    async def get_tenant_query_history(
        ...
        try:
            async for db in get_platform_db():
                ...
                query = select(GatewayQueryLog).where(
                    GatewayQueryLog.tenant_id == tenant_id
                )

                if database_id:
                    query = query.where(GatewayQueryLog.database_id == database_id)
                if user_id:
                    query = query.where(GatewayQueryLog.user_id == user_id)
                if status:
                    query = query.where(GatewayQueryLog.status == status)
                if start_date:
                    query = query.where(GatewayQueryLog.requested_at >= start_date)
                if end_date:
                    query = query.where(GatewayQueryLog.requested_at <= end_date)
                ...
                query = query.order_by(desc(GatewayQueryLog.requested_at))
    ...
    async def get_tenant_analytics(
        ...
        try:
            async for db in get_platform_db():
                base_query = select(GatewayQueryLog).where(
                    GatewayQueryLog.tenant_id == tenant_id
                )

                if start_date:
                    base_query = base_query.where(GatewayQueryLog.requested_at >= start_date)
                if end_date:
                    base_query = base_query.where(GatewayQueryLog.requested_at <= end_date)
                ...
                # Success count
                success_query = base_query.where(
                ...
                # Error count
                error_query = base_query.where(
                    GatewayQueryLog.status == QueryStatus.ERROR.value
                )
                error_result = await db.execute(
                    select(func.count()).select_from(error_query.subquery())
                )
                error_count = error_result.scalar()

                # Average execution time
                avg_time_result = await db.execute(
                    select(func.avg(GatewayQueryLog.execution_time_ms)).where(
                        GatewayQueryLog.tenant_id == tenant_id,
                ...
                tokens_result = await db.execute(
                    select(func.sum(GatewayQueryLog.tokens_used)).where(
                        GatewayQueryLog.tenant_id == tenant_id,
...
async def log_query(**kwargs) -> Optional[str]:
    """Log a query execution."""
    service = get_query_logging_service()
    return await service.log_query(**kwargs)


async def update_query_result(**kwargs) -> bool:
    """Update a query log with results."""
    service = get_query_logging_service()
    return await service.update_query_result(**kwargs)
