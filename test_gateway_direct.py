"""
Gateway WebSocket Direct Test (bypassing IIS)
Tests direct connection to backend on port 5005
"""
...
async def test_gateway_websocket():
    """Test the gateway WebSocket endpoint directly (bypassing IIS)"""

    # Direct to backend on port 5005
    ws_url = "ws://103.197.77.163:5005/api/gateway/ws"

    print("=" * 60)
    print("Gateway WebSocket Direct Test (Port 5005)")
    print("=" * 60)
    ...
    try:
        ...
        print("   Port 5005 may not be accessible from outside the VM")
        return False
...
async def main():
    """Run all tests"""
    print("\n" + "=" * 60)
    print(" DIRECT GATEWAY TEST (bypassing IIS)")
    print("=" * 60)
